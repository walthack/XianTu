...
2. 在首页进入“剧本模组”。
3. 导入文件，查看内容预览、错误与警告。
4. 无错误时确认导入；同 ID 文件会显示被替换的本地版本。
5. 进入“独自修行”并开始新角色。
6. 在世界选择页选择 Strict 或 Expand Mod。

管理器可下载 Strict/Expand 模板，并在导入前展示世界、章节、事件、人物、势力、地点、技能、功法、物品、玩家身份、专属内容及人物归属。错误会阻止确认，警告允许作者复核后导入；诊断结果可导出为 JSON 报告。导入成功后可直接进入创角。
...
| 重要人物 | `canon.characters` | 进入 Mod 正典和剧情上下文；被关系字段引用时创建原生 NPC 档案 |
| 玩家关系与好感 | `canon.playerRelationships` | 初始化原生 `社交.关系` NPC 档案、关系类型、好感度与记忆 |
| NPC 人际关系 | `canon.relationships` | 初始化原生 `社交.关系矩阵.edges` |
...
`content` 描述世界中存在的定义。人物通过 `skillIds`、`techniqueIds`、`itemIds` 引用后，若该人物被初始关系引用，系统会把技能、功法和物品映射进其原生 NPC 档案；若玩家通过 `playerCharacterId` 扮演该正典人物，则同步写入玩家原生技能、修炼功法和背包。独立玩家不会自动获得 NPC 的内容，仍应通过剧情和正常游戏指令取得。

人物的 `affiliations` 会映射到 NPC 的原生势力与宗门字段。只有玩家映射为正典人物且该人物存在 `sect` 归属时，才初始化玩家的 `社交.宗门`；独立玩家不会因为 Mod 中存在宗门而自动入门。

### 玩家身份与专属内容

`scenario.opening.playerCharacterId` 可把玩家映射为一个 `canon.characters` 正典人物。未填写时，玩家是独立身份，不会自动继承任何正典人物的专属能力。

正典人物路线可在 `scenario.opening.creationPreset` 声明完整创角预制。选择 Mod 后，第 2–7 步会显示 Mod 的天资、出身、灵根或特殊体质、天赋、先天六司与身份，不再展示通用世界模板；锁定预制会跳过天道点预算并禁止修改身份字段。没有 `creationPreset` 的 Mod 保持原有自由创角流程。
...
导入时 validator 会拒绝不支持的 Schema、重复 ID、悬空引用、不安全路径和无效条件。CLI 在结构校验通过后还会运行静态可玩性分析：

- `ERROR` 会令命令返回失败：未初始化 flag、没有可激活的开场章节、非终章没有完成条件、章节依赖环。
- `WARNING` 不令命令失败，但交付前应人工确认：没有章节、孤立事件、事件跨章节复用、无法从已知章节完成条件推导到的不可达章节。

章节依赖由“前章 completion 条件与后章 activation 条件一致”推导。运行时允许 LLM 根据剧情设置 flags，因此分析器只证明显式状态机链路，不尝试理解自然语言剧情。建议从最小 fixture 复制一份，再逐步添加正典与剧情内容，并以零警告作为正式 Mod 的交付目标。
