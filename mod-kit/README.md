...
原作唯一能力或限定传承使用 `rules.contentAccess` 声明 `exclusive` / `restricted` 持有者；玩家扮演正典人物时使用 `scenario.opening.playerCharacterId`，并用 `creationPreset` 提供第 2–7 步的正典创角数据。完整规则见 [`AGENT_GUIDE.md`](./AGENT_GUIDE.md#正典身份与专属内容)。
